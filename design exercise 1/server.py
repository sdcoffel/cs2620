...
def delete_message_handler(connection, message_content):
    ...
    if delete_message(message_content, MESSAGES_FILE_PATH):
        ...
        connection.send(
            f"Message with content '{message_content}' not found.".encode("utf-8")
        )
...
def handle_account_deletion(connection, username):
    ...
    if username in pending_messages and pending_messages[username]:
        ...
        if confirmation == "yes":
            ...
            delete_pending_messages(
                PENDING_MESSAGES_FILE_PATH, username, len(pending_messages[username])
            )  # deletes all pending messages as well as the account itself
            ...
            if JSON_MODE:
                connection.send(
                    json.dumps({"data": "Account deletion aborted."}).encode("utf-8")
                )
    ...
    if JSON_MODE:
        connection.send(
            json.dumps({"data": "Account deleted successfully."}).encode("utf-8")
        )
...
def start_server():
    ...
    try:
        ...
        server_socket.bind(
            ("0.0.0.0", port)
        )  # listens on all interfaces for the client
